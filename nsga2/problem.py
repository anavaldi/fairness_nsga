...
class Problem:
    ...
    def calculate_objectives(self, individual, first_individual, seed):
        if self.expand:
            ...
            # problem with 3 obj func:
            #individuals_aux = pd.DataFrame({'id': individual.id, 'creation_mode':individual.creation_mode, 'criterion': [criterion], 'max_depth': [max_depth], 'min_samples_split': [min_samples_split], 'max_leaf_nodes': [max_leaf_nodes], 'class_weight': [class_weight], 'error': error, 'dem_fp': dem_fp, 'complexity': leaves, 'actual_depth': depth, 'actual_leaves': leaves})
            ...
            # problem with 3 obj func:
            self.individuals_df.to_csv('./results/individuals/individuals_' + self.dataset_name + '_seed_' + str(seed) + '_gen_' + self.variable_name + '_indiv_' + str(self.num_of_generations) + '_' + str(self.num_of_individuals) + '.csv', index = False, header = True, columns = ['id', 'creation_mode', 'error', 'dem_fp', 'complexity', 'criterion', 'max_depth', 'min_samples_split', 'max_leaf_nodes', 'class_weight', 'actual_depth', 'actual_leaves'])
