...
class Evolution:
    ...
    def __init__(self, problem, results_df, num_of_generations=5, num_of_individuals=10, num_of_tour_particips=2, tournament_prob=0.9, crossover_param=2, mutation_param=5, mutation_prob=0.1, beta_method="uniform"):
    ...
    def evolve(self):
        ...
        for i in range(self.num_of_generations):
            for indiv in self.population.population:
                results_aux = pd.DataFrame({'individual': [indiv.features], 'error': indiv.objectives[0], 'dem_fp': indiv.objectives[1], 'generation': i})
                self.results_df = self.results_df.append(results_aux, ignore_index = True) 
            if i == (self.num_of_generations-1):
                self.results_df.to_csv("./results/results_prueba_german_age_" + str(self.num_of_generations) + "_" + str(self.num_of_individuals) +  ".csv", index = False, header = False)
            print("GENERATION:")
            print(i)
            print("FEATURES:")
