    ...
    def evolve_log(self):
        ...
        for i in range(self.num_of_generations):
            for indiv in self.population.population:
                ...
                evolutions_aux = pd.DataFrame({'max_iter': [max_iter], 'tol': [tol], 'C': [C], 'l1_ratiofloat': [l1_ratiofloat], 'class_weight': [class_weight],  'error': indiv.objectives[0], 'dem_fp': indiv.objectives[1], 'generation': i+1, 'rank': indiv.rank, 'id': indiv.id, 'creation_mode': indiv.creation_mode, 'seed': self.utils.problem.seed})
                self.evolutions_df = pd.concat([self.evolutions_df, evolutions_aux])
            if i == (self.num_of_generations-1):
                self.evolutions_df.to_csv("./results/population/evolution_" + self.dataset_name + "_" + self.protected_variable + '_seed_' + str(self.utils.problem.seed) + "_gen_" + str(self.num_of_generations) + "_indiv_" + str(self.num_of_individuals) +  ".csv", index = False, header = True, columns = ['seed', 'id', 'generation', 'rank', 'creation_mode', 'error', 'dem_fp', 'max_iter', 'tol', 'C', 'l1_ratiofloat', 'class_weight'])
            print("GENERATION:",i)
            self.population.extend(children)
            self.utils.fast_nondominated_sort(self.population)
            new_population = Population()
